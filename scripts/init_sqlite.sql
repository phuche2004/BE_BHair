...
-- Full-text search for shops (FTS5 virtual table)
...
CREATE TRIGGER IF NOT EXISTS shops_fts_insert AFTER INSERT ON shops BEGIN
  INSERT INTO shops_fts(rowid, id, name, address)
  VALUES (new.rowid, new.id, new.name, new.address);
END;
...
CREATE TRIGGER IF NOT EXISTS shops_fts_delete AFTER DELETE ON shops BEGIN
  DELETE FROM shops_fts WHERE rowid = old.rowid;
END;
