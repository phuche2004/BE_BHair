...
export function CreateAppointmentModal({ visible, onClose, onSuccess, selectedDate, initialTime }: CreateAppointmentModalProps) {
  ...
  const handleCreate = async () => {
    ...
    if (!initialTime) return Alert.alert('Lỗi', 'Không có giờ được chọn');
    if (selectedServices.length === 0) return Alert.alert('Lỗi', 'Vui lòng chọn ít nhất 1 dịch vụ');

    setSubmitting(true);
    try {
       const dateStr = selectedDate || new Date().toISOString().split('T')[0];
       const bookingDate = new Date(`${dateStr}T${initialTime}:00`);
          ...
          <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
             ...
             {loading ? <ActivityIndicator style={{ marginTop: 40 }} color={colors.primary} /> : (
               <ScrollView style={styles.modalBody} contentContainerStyle={{ paddingBottom: 40 }}>
                 ...
                 <Text style={[styles.label, { color: colors.primary, marginTop: 24 }]}>
                    Thời gian: <Text style={{ color: colors.secondary }}>{initialTime} - {selectedDate}</Text>
                 </Text>
